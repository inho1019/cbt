...
const ncp207 = [
    {
        question: 'PS 명령어 실행시 STAT 필드에 대한 설명 중 잘못된 것은?',
        answer: [
            { text: 'R: running or runnable', type: false },
            { text: 'Z: defunct (“zombie”) process', type: false },
            { text: 'W: paging', type: false },
            { text: 'S: stopped, either by a job control signal or because it is being traced', type: true },
        ],
    },
    {
        question: '리눅스에서 네트워크 트래픽을 확인할 수 있는 명령어를 고르시오.',
        answer: [
    ...
    {
        question: '리눅스 서버에서 서버의 자체 네트워크에 문제가 없는 확인하려 합니다. 스스로에게 5 번 ping을 날리고자 할 때 올바른 명령어는?',
        answer: [
            { text: 'ping -n 5 localhost', type: false },
            { text: 'ping -c 5 localhost', type: true },
            { text: 'ping -l localhost', type: false },
    ...
    {
        question: '다수의 클라이언트에서 서버의 공인 IP로 접속하면 잘 보이지만 L7 헬스체크가 적용된 LB로 접속하면 접속이 되지 않고 LB에서 보면 서버 연결상태가 실패로 나온다. 원인으로 추정할만한 것은 무엇인가?',
        answer: [
            { text: 'ACG', type: true },
    ...
    {
        question: 'Windows 환경에서 다음의 ping 명령어의 옵션은 어떤 의미인가? ping -n 5 -i 2 www.naver.com',
        answer: [
            { text: '5 byte를 2 초 마다 전송', type: false },
            { text: '2 초의 간격으로 5 번 전송', type: false },
            { text: 'TTL 값을 2로 하여 5 번 전송', type: true },
            { text: '5 byte를 2 번 전송', type: false },
        ],
    },
    {
        question: 'SAR로 물리적 디스크에서 발생된 I/O를 확인하고자 한다. 이 때 사용할 수 있는 옵션은 무엇인가?',
        answer: [
    ...
    {
        question: '스냅샷 이미지를 이용해서 신규 생성한 볼륨을 현재 사용 중인 서버에 attach한 후 서버를 리부팅하였으나, 정상적으로 리부팅이 되지 않는다. 어떤 이슈를 의심할 수 있는가?',
        answer: [
            { text: '해당 스냅샷 이미지는 sysprep 작업이 적용되지 않은 이미지일 것이다.', type: false },
            { text: '스냅샷 이미지를 이용해 생성한 볼륨을 서버에 마운트 후 리부팅을 했어야 한다.', type: false },
            { text: '스냅샷 이미지를 이용해 생성한 볼륨이 다른 서버의 부팅 볼륨일 것이며, uuid 중복으로 정상 부팅되지 않는 것이다.', type: false },
            { text: '스냅샷 이미지를 이용해서 신규 볼륨 생성 시 선택한 볼륨 타입이 현재 사용 중인 서버에 attach할 수 없는 볼륨이다.', type: true },
        ],
    },
    {
    ...
    {
        question: '네이버 클라우드 플랫폼에서 제공되는 웹 서능 측정 도구는 무엇인가?',
        answer: [
            { text: 'AB', type: false },
    ...
    {
        question: 'Classic 플랫폼에서 서버 상품을 이용중입니다 서버에 IP를 하나 더 부여해야 하는 상황이 되었습니다. 이 때 올바른 사용 방법은 무엇인가요?',
        answer: [
            { text: 'Private Subnet을 생성한 후 network Interface를 부착한다', type: false },
            { text: 'Network Interface를 단독 생성하여 부착한다', type: true },
            { text: '기존 사용하던 Network Interface에 Secondary IP를 추가한다', type: false },
    ...
    {
        question: '리눅스에서 네트웍 트래픽을 확인할 수 있는 명령어를 모두 고르시오.',
        answer: [
            { text: 'netstat -l', type: true },
            { text: 'ifconfig', type: true },
            { text: 'netconfig', type: false },
            { text: 'network', type: false },
        ],
    ...
    {
        question: '스냅샷 이미지를 이용해서 신규 생성한 볼륨을 현재 사용 중인 서버에 attach 한 후 서버를 리부팅하였으나, 정상적으로 리부팅이 되지 않는다. 어떤 이슈를 의심할 수 있는가?',
        answer: [
            { text: '해당 스냅샷 이미지는 sysprep 작업이 적용되지 않은 이미지일 것이다.', type: false },
            { text: '스냅샷 이미지를 이용해 생성한 볼륨을 서버에 마운트 후 리부팅을 했어야 한다.', type: false },
            { text: '스냅샷 이미지를 이용해 생성한 볼륨이 다른 서버의 부팅 볼륨일 것이며, 이에 중복으로 정상 부팅이 되지 않는 것이다.', type: true },
            { text: '스냅샷 이미지를 이용해서 신규 볼륨 생성 시 선택한 볼륨 타입이 현재 사용 중인 서버에 attach 할 수 없는 볼륨이다.', type: false },
        ],
    },
    {
    ...
    {
        question: '리눅스 서버에서 서버의 자체 네트워크에 문제가 없는지 확인하려 합니다. 스스로에게 5 번 ping을 날리고자 할 때 올바른 명령어는?',
        answer: [
            { text: 'ping -n 5 localhost', type: false },
            { text: 'ping -c 5 localhost', type: true },
            { text: 'ping -1 localhost', type: false },
            { text: 'ping -t 5 localhost', type: false },
        ],
    },
    {
        ...
        answer: [
            { text: '5 byte를 2 초 마다 전송', type: false },
            { text: '2 초의 간격으로 5 번 전송', type: false },
            { text: 'TLL 값을 2로 하여 5 번 전송', type: true },
            { text: '5 byte를 2 번 전송', type: false },
        ],
    },
    {
    ...
    {
        question: '네이버 클라우드 플랫폼 Classic 환경에 존재하는 서버의 10.X.X.X 사설 IP 대역 NIC에 두 개 이상의 IP를 부여하였을 때 발생할 수 있는 상황은?',
        answer: [
            { text: '기존 IP로는 통신이 되나 추가된 IP로 통신이 되지 않는다.', type: false },
            { text: '추가된 IP 뿐만 아니라 기존 PE 통신이 되지 않는다.', type: true },
    ...
    },
    {
        ...
        answer: [
            { text: '200 - 서버가 요청을 제대로 처리', type: false },
            { text: '300 - 서버가 GET 요청의 일부 성공적으로 처리했다.', type: true },
            { text: '404 - 서버가 요청한 페이지(Resource)를 찾을 수 없다', type: false },
            { text: '500 - 서버에 오류가 발생하여 요청을 수행할 수 없다.', type: false },
        ],
    },
    ...
    },
    {
        ...
        answer: [
            ...
            { text: '5 번 응답이 없는 경우 Unbind 한다.', type: true },
            { text: 'Unbind 됐던 서버가 헬스체크 응답이 3 번 성공하면 Bind 한다.', type: false },
            { text: 'VPC 환경에서는 헬스체크 주기를 Target Group 에서 변경할 수 있다.', type: false },
        ],
    },
    {
    ...
    {
        question: '리눅스 서버를 운영중에 동접이 2000 정도에서 더이상 새로운 접속을 맺을 수 없는 현상을 발견하였다. 어느 부분을 의심하여야 할까?',
        answer: [
            { text: 'ACG', type: false },
            { text: '/etc/sysctl.conf 파일에서 최대 오픈 파일 수 확인', type: false },
            { text: '/etc/sysctl.conf 파일에서 최대 접속 가능 범위 확인', type: false },
            { text: 'Apache 의 Max Client', type: true },
        ],
    },
    {
    ...
    {
        question: '네이버 클라우드 플랫폼에서 제공되는 웹 성능 측정 도구는 무엇인가?',
        answer: [
            { text: 'AB', type: false },
            { text: 'ngrinder', type: true },
            { text: 'topaz', type: false },
            { text: 'Ioadrunner', type: false },
        ],
    },
    {
    ...
    {
        question: '리눅스에서 ICMP 와 TTL 을 이용하여 경로를 확인할 수 있는 프로그램은?',
        answer: [
            { text: 'Traceroute', type: true },
            { text: 'Tracert', type: false },
            { text: 'Tcpdump', type: false },
            { text: 'Topview', type: false },
        ],
    },
    {
    ...
    {
        question: '"su root" 명령어와 "su -root"명령어의 차이점에 대해 올바른 설명은?',
        answer: [
            { text: '차이없음', type: false },
            { text: 'su -명령어는 root 계정의 환경변수를 그대로 가져오지만, su root는 기존 계정의 환경변수를 사용한다', type: true },
            { text: 'su 명령어는 sudo로 실행 가능한 명령어만 실행 가능하다', type: false },
            { text: 'su명령어로 계정을 변경하게 되면 exit 시 로그아웃하게 된다.', type: false },
        ],
    },
    {
    ...
    {
        question: '서버생성 시, init script를 적용하였으나 제대로 반영이 되지 않은 것 같은 경우 init script에 대한 로그를 확인할 수 있는 경로로 알맞은 것은? (리눅스 기준)',
        answer: [
            { text: '/var/log/ncloud.log', type: false },
            { text: '/var/log/ncloud/init.log', type: false },
            { text: '/var/log/ncloud/initscript.log', type: false },
    ...
    },
    {
        ...
        answer: [
            { text: 'lsof -P', type: true },
            { text: 'lsof -n', type: true },
            { text: 'lsof -w', type: false },
            { text: 'lsof -h', type: true },
        ],
    },
    ...
    },
    {
        ...
        answer: [
            ...
            { text: '이벤트 뷰어 > Windows 로그 > 시스템', type: false },
            { text: '이벤트 뷰어 > Windows 로그 > Setup', type: false },
            { text: '이벤트 뷰어 > Windows 로그 > Forwarded Event', type: false },
        ],
    },
    {
        question: 'CentOS 6.x 이상에서는 스토리지 식별을 무엇으로 하나요?',
